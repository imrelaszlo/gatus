...
func (provider *AlertProvider) buildMessageSubjectAndBody(cfg *Config, ep *endpoint.Endpoint, alert *alert.Alert, result *endpoint.Result, resolved bool) (string, string, string) {
	...
	if len(cfg.EmailFileTemplateTriggered) > 0 && !resolved {
		body_html = provider.replaceBodyPlaceholders(ep, alert, result, cfg.EmailFileTemplateTriggered, strings.ReplaceAll(formattedConditionResults, "\n", "<br>"))
		body_text = strip.StripTags(body_html)
		return subject, body_text, body_html
	}
	if len(cfg.EmailFileTemplateResolved) > 0 && resolved {
		body_html = provider.replaceBodyPlaceholders(ep, alert, result, cfg.EmailFileTemplateResolved, strings.ReplaceAll(formattedConditionResults, "\n", "<br>"))
		body_text = strip.StripTags(body_html)
		return subject, body_text, body_html
	}
	// If no HTML file is specified, use the text overrides from configuration
	if len(cfg.TextEmailBodyTriggered) > 0 && !resolved {
		body_text = provider.replaceBodyPlaceholders(ep, alert, result, cfg.TextEmailBodyTriggered, formattedConditionResults)
		return subject, body_text, ""
	}
	if len(cfg.TextEmailBodyResolved) > 0 && resolved {
		body_text = provider.replaceBodyPlaceholders(ep, alert, result, cfg.TextEmailBodyResolved, formattedConditionResults)
		return subject, body_text, ""
	}
	// Fallback to the default message body if no overrides are specified
	var description string
	if alertDescription := alert.GetDescriptionUseResponseBody(result.Body); len(alertDescription) > 0 {
		description = "\n\nAlert description: " + alertDescription
	}
	return subject, body_text + description + formattedConditionResults, body_html
}

func (provider *AlertProvider) replaceBodyPlaceholders(ep *endpoint.Endpoint, alert *alert.Alert, result *endpoint.Result, str string, formattedConditionResults string) string {
